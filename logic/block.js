...
Block.prototype.getId = function (block) {
	var blockHash = crypto.createHash('sha1').update(this.getBytes(block)).digest();
	// var temp = new Buffer(8);
	// for (var i = 0; i < 8; i++) {
	// 	temp[i] = hash[7 - i];
	// }

	var id = blockHash.toString('hex')
	return id;
}

Block.prototype.getHash = function (block) {
	return crypto.createHash('sha1').update(this.getBytes(block)).digest();
}
