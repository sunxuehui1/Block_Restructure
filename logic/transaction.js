...
Transaction.prototype.getId = function (trs) {
	var trsHash = this.getHash(trs);
	// var temp = new Buffer(8);
	// for (var i = 0; i < 8; i++) {
	// 	temp[i] = hash[7 - i];
	// }
	var id = trsHash.toString('hex')
	// var id = bignum.fromBuffer(temp).toString();
	return id;
}

Transaction.prototype.getHash = function (trs) {
	return crypto.createHash('sha1').update(this.getBytes(trs)).digest();
}
...
Transaction.prototype.getBytes = function (trs, skipSignature, skipSecondSignature) {
	...
	try {
		...
		bb.writeUTF8String(trs.timestamp);
...
Transaction.prototype.verify = function (trs, sender, requester, cb) { //inheritance
	accounts.prototype.setAccountAndGet({publicKey: trs.senderPublicKey}, function (err, sender) {
		if (typeof requester === 'function') {
		cb = requester;
	}
	...
	try {
		...
		if (trs.requesterPublicKey) {
			valid = Transaction.prototype.verifySignature(trs, trs.requesterPublicKey, trs.signature);
		} else {
			valid = Transaction.prototype.verifySignature(trs, trs.senderPublicKey, trs.signature);
		}
	} catch (e) {
	...
	if (!trs.requesterPublicKey && sender.secondSignature) {
		try {
			var valid = Transaction.prototype.verifySecondSignature(trs, sender.secondPublicKey, trs.signSignature);
		} catch (e) {
			return setImmediate(cb, e.toString());
		}
		if (!valid) {
		...
		try {
			var valid = Transaction.prototype.verifySecondSignature(trs, requester.secondPublicKey, trs.signSignature);
		} catch (e) {
			return setImmediate(cb, e.toString());
		}
		if (!valid) {
	...
	if (trs.signatures) {
		for (var d = 0; d < trs.signatures.length; d++) {
			...
			for (var s = 0; s < multisignatures.length; s++) {
				...
				if (Transaction.prototype.verifySignature(trs, multisignatures[s], trs.signatures[d])) {
					verify = true;
				}
			}
	...
	//}
	if (trs.amount < 0 || trs.amount > 100000000 * constants.fixedPoint || trs.amount.toString().indexOf('e') >= 0) {
		return setImmediate(cb, "Invalid transaction amount: " + trs.id);
	}
	// Check timestamp，将标准时间转化为验证时间戳
	//if (slots.getSlotNumber(trs.timestamp) > slots.getSlotNumber()) {
	//	return setImmediate(cb, "Invalid transaction timestamp");
	//}
	if (slots.getSlotNumber(slots.getTime( (new Date(trs.timestamp)).getTime())) > slots.getSlotNumber()) {
		return setImmediate(cb, "Invalid transaction timestamp");
	}
